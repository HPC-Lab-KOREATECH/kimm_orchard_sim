...
public:
    ...
    // localization -----------------------------
    // TODO : change global map ->> surf, corner 
    void loadGlobalMap()
    {        
        pcl::io::loadPCDFile<PointType>(map_dir + "CornerMap.pcd", *laserCloudCornerFromMap);
        downSizeFilterCorner.setInputCloud(laserCloudCornerFromMap);
        downSizeFilterCorner.filter(*laserCloudCornerFromMapDS);

        laserCloudCornerFromMapDSNum = laserCloudCornerFromMapDS->size();
        std::cout << "global map size (Corner) : " << laserCloudCornerFromMapDSNum << std::endl;

        pcl::io::loadPCDFile<PointType>(map_dir + "SurfMap.pcd", *laserCloudSurfFromMap);
        downSizeFilterCorner.setInputCloud(laserCloudSurfFromMap);
        downSizeFilterCorner.filter(*laserCloudSurfFromMapDS);

        laserCloudSurfFromMapDSNum = laserCloudSurfFromMapDS->size();
        std::cout << "global map size (Surface) : " << laserCloudSurfFromMapDSNum << std::endl;
    ...
    void publishFrames()
    {
        ...
        if (pubRecentKeyFrame->get_subscription_count() != 0 && doMapping)
        {
            pcl::PointCloud<PointType>::Ptr cloudOut(new pcl::PointCloud<PointType>());
        ...
        if (pubCloudRegisteredRaw->get_subscription_count() != 0 && doMapping)
        {
            pcl::PointCloud<PointType>::Ptr cloudOut(new pcl::PointCloud<PointType>());
