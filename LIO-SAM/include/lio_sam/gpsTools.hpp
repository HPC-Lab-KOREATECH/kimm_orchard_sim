...
// UTM 
void convertGeodeticToUTM(double latitude, double longitude, double &northing, double &easting, int &zone) {
    
    double a = 6378137; // WGS 84 Equatorial axis
    double e = 0.081819191; // WGS 84 eccentricity
    double k0 = 0.9996; // UTM 스케일 팩터
    int zone_number;
    ...
    double lambda0 = deg2rad(central_meridian);

    double N = a / sqrt(1 - e * e * sin(phi) * sin(phi));
    double T = tan(phi) * tan(phi);
    double C = e * e * cos(phi) * cos(phi);
    ...
    northing = (double)(k0 * (M + N * tan(phi) * (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * pow(A, 4) / 24 +
                 (61 - 58 * T + T * T + 270 * C - 330 * e * e * sin(phi) * sin(phi)) * pow(A, 6) / 720)));

    easting = (double)(k0 * (N * (A + (1 - T + C) * pow(A, 3) / 6 +
                 (5 - 18 * T + T * T + 72 * C - 58 * e * e) * pow(A, 5) / 120) +
                 500000.0));

    // 남반구일 경우 northing을 추가로 계산
    if (latitude < 0)
        northing += 10000000.0;
