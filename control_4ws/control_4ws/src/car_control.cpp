...
public:
    ...
    void timer_callback()
    {
        ...
        float curr_speed = sqrt((cb_data->get_vx_km(), 2) + pow(cb_data->get_vx_km(), 2));
        ...
        if ((abs(l_n_a.linear) < 0.6) && (abs(l_n_a.angular) < 0.6)){
            c_mode = false;
        }
        ...
        if (c_mode){
            ...
            angular_z = (angular_z + (angular_z / max(abs(angular_z), 0.01))* abs(linear_y) * 1.5 ); //+ linear_y - ff_controlv  
            angular_z = (angular_z -(linear_y) * 1.5 );
            linear_y = 0; 
        }
